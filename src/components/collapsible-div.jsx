...
const CollapsibleDiv = ({id, header, children, className}) => {
  ...
  return (
    <div key={id} className={`${className} rounded-sm flex flex-col`}>
        <Button type="button" onClick={() => growDiv(id)} className={`${divOpen ? '' : 'shadow-md hover:shadow-sm shadow-tertiary hover:translate-y-0.5 transition-all duration-200'} flex`}>
            {header}
        </Button>
